...
from utils import EpisodicBatchSampler, euclidean_dist, norm_dist
from protonet import Protonet

def eval_prototypes(conf=None, hdf_eval=None, strt_index_query=None, logger=None):
    ...
    Model.load_from_checkpoint(conf.path.last_model, conf=conf)
    ...
    for i in range(iterations):
        ...
        for batch in tqdm(neg_iterator, desc="NEG Proto"):
            x_neg, y_neg = batch
        ...
        neg_prototype = neg_set_feat.mean(dim=0)
        neg_prototype = neg_prototype.to(device)

        for batch in tqdm(pos_iterator, desc="POS Proto"):
            x_pos, y_pos = batch
        ...
        pos_prototype = pos_set_feat.mean(dim=0)
        pos_prototype = pos_prototype.to(device)

        for batch in tqdm(q_iterator, desc="Query    "):
            x_q, y_q = batch
            x_q = x_q.to(device)
            x_query = Model(x_q)
            probability_pos = get_probability(pos_prototype, neg_prototype, x_query)
